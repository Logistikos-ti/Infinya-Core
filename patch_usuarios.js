const fs = require('fs');
const file = 'src/components/configuracoes/infinoos-configuracoes-view.tsx';
let code = fs.readFileSync(file, 'utf8');

const usuariosCustomTable = `
        {/* ============ TAB: USUÁRIOS (Custom Table Design) ============ */}
        {tab === "usuarios" && (
          <div className="overflow-x-auto rounded-[16px] border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-zinc-950/40">
            <div className="min-w-[900px]">
              ...
              {(initialUsuarios || []).map((item, i) => {
                const isCurrentUser = false; // Add actual logic if needed
                  ...
                  <div key={item.id} className="flex items-center gap-4 border-b border-slate-100 px-5 py-4 transition hover:bg-slate-50/50 dark:border-white/5 dark:hover:bg-white/[0.02] last:border-0">
                    <div className="flex flex-[2.2] items-center gap-3 min-w-0">
                      ...
                      <div className="flex flex-col gap-0.5 min-w-0">
                        <span className="truncate text-sm font-bold text-slate-900 dark:text-slate-100">{item.nome || item.email?.split('@')[0]}</span>
                        <span className="truncate text-xs text-slate-500 dark:text-slate-400">{item.email || "Sem login"}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-[1.6] items-center min-w-0">
                      <span className="truncate text-[13px] text-slate-900 dark:text-slate-100">
                        {item.depositante || "Sem depositante"}
                      </span>
                    </div>

                    <span className="flex-[1.2] text-[12.5px] text-slate-500 dark:text-slate-400">
                      {item.ultimo_acesso_em ? new Date(item.ultimo_acesso_em).toLocaleDateString("pt-BR") : "Nunca"}
                    </span>
              ...
              {(!initialUsuarios || initialUsuarios.length === 0) && (
                <div style={{ padding: '40px', textAlign: 'center', color: t.textSub, fontSize: '14px' }}>Nenhum usuário cadastrado.</div>
              )}
            </div>
          </div>
        )}
...
code = code.replace('{/* ============ TAB: USUÁRIOS / TRANSPORTADORAS (Table rows) ============ */}', usuariosCustomTable + '\n        {/* ============ TAB: USUÁRIOS / TRANSPORTADORAS (Table rows) ============ */}');

-- New order-lifecycle status for the mandatory scan-to-return cancellation
-- flow: an order sits here while a cancellation is EM_ANDAMENTO, instead of
-- leaving pedidos_expedicao.status untouched (the original gap the user
-- flagged -- nothing else in the app could tell an order was mid-cancel).
-- Exits only via concluir_cancelamento_pedido_expedicao (-> CANCELADO) or
-- abandonShippingOrderCancellationAction (-> restored to
-- status_pedido_na_abertura).
alter type public.status_pedido_expedicao add value if not exists 'EM_CANCELAMENTO';
commit;

-- Same body as reservar_item_pedido_expedicao in
-- supabase/migrations/20260821153000_reserve_shipping_stock_on_order_creation.sql:161-303,
-- with EM_CANCELAMENTO added to the early-return exclusion list so a
-- resync (e.g. Bling reposting the order) while a cancellation is pending
-- can't trigger a live stock reservation on an order that's about to be
-- cancelled anyway.
...
begin
  ...
  if v_item.status in ('CANCELADO', 'CONFERIDO', 'PRONTO_ROMANEIO', 'EXPEDIDO', 'EM_CANCELAMENTO') then
    return 0;
  end if;

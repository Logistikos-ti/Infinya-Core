...
function buildShippingLabelZpl(order: ShippingOrderDetail) {
  const lines = [
    ...
    "^LL1218",
    "^LH0,0",
    "^FO40,30^A0N,34,34^FDINFINOOS WMS - ETIQUETA DE TRANSPORTE^FS",
    ...
    `^FO40,920^A0N,24,24^FDGerado por Infinoos WMS em ${sanitizeZpl(order.syncedAt)}^FS`,
...
function buildShippingLabelPdf(order: ShippingOrderDetail) {
  const contentLines = [
    "INFINOOS WMS - ETIQUETA DE TRANSPORTE",
    "",
    ...
    "",
    `Gerado por Infinoos WMS em ${order.syncedAt}`,
  ];
...
function sanitizeBarcode(value: string) {
  return value.replace(/[^A-Za-z0-9\-]/g, "").slice(0, 32) || "INFINOOSWMS";
}

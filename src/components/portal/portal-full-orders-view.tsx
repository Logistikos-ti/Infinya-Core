    ...
    <aside className="relative flex h-full w-full max-w-[620px] flex-col bg-white shadow-2xl dark:bg-[#0c1424]">
      ...
      <form action={action} className="flex min-h-0 flex-1 flex-col">
        <div className="flex-1 space-y-6 overflow-y-auto px-6 py-6">
          ...
          <section><p className="mb-2 text-xs font-bold text-slate-500">Marketplace</p><div className="flex flex-wrap gap-2">{marketplaces.map((item) => <button type="button" onClick={() => setMarketplace(item)} key={item} className={`rounded-xl border px-3 py-2 text-xs font-bold ${item === marketplace ? "border-violet-500 bg-violet-50 text-violet-700 dark:bg-violet-400/10 dark:text-violet-200" : "border-slate-200 text-slate-600 dark:border-white/10 dark:text-slate-300"}`}>{item}</button>)}</div></section>
          ...
          <Attachment field="entryAuthorization" label="Autorização de entrada *" /><Attachment field="volumeLabel" label="Etiqueta de volume *" />
          {minimumLabels.length ? <section className="space-y-3"><h3 className="font-bold">Etiquetas por produto *</h3><p className="text-xs text-slate-500">Cada item precisa do arquivo de etiqueta usado pelo centro de fulfillment.</p>{items.map((item) => <label key={item.key} className="block rounded-xl border border-slate-200 p-3 text-sm dark:border-white/10"><strong>{item.name}</strong><span className="ml-2 text-xs text-slate-500">{item.code} · {item.quantity} un</span><input name="itemLabel" required type="file" accept=".pdf,.png,.jpg,.jpeg,application/pdf,image/png,image/jpeg" className="mt-2 block text-xs" /></label>)}</section> : null}

...
function buildRawXmlPreviewHtml(xml: string) {
  return `<!doctype html><html><head><meta charset="utf-8"><style>html,body{margin:0;background:#fff;color:#111827;font-family:ui-monospace,SFMono-Regular,Consolas,monospace}body{padding:24px}pre{margin:0;white-space:pre-wrap;overflow-wrap:anywhere;font-size:12px;line-height:1.6}</style></head><body><pre>${escapePreviewHtml(xml)}</pre></body></html>`;
}
                ...
                <section>
                  ...
                  <div style={{ display: "grid", gap: 10 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: 14, borderRadius: 12, border: `1px solid ${t.border}`, background: t.softBg }}>
                      ...
                      {newOrderInvoiceFile && <button type="button" onClick={async () => { setNewOrderPreviewZoom(100); setNewOrderPreview({ kind: "invoice", src: buildRawXmlPreviewHtml(await newOrderInvoiceFile.text()), file: newOrderInvoiceFile }); }} style={{ height: 34, padding: "0 10px", borderRadius: 9, border: `1px solid ${t.border}`, background: t.cardBg, color: t.text, fontSize: 12, fontWeight: 800, cursor: "pointer" }}>Visualizar</button>}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: 14, borderRadius: 12, border: `1px solid ${t.border}`, background: t.softBg }}>
            ...
            {newOrderPreview && <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 16, background: "rgba(15,23,42,.72)", animation: "overlayFade .2s ease" }}>
              <div style={{ width: "min(1150px, 100%)", height: "min(92%, 900px)", overflow: "hidden", display: "flex", flexDirection: "column", borderRadius: 22, background: "#fff", boxShadow: "0 24px 80px rgba(0,0,0,.35)" }}>
                ...
                <div style={{ flexShrink: 0, height: 56, display: "flex", alignItems: "center", gap: 18, padding: "0 20px", background: "#3B3B3B", color: "#fff" }}>
                  <span style={{ fontSize: 18 }}>☰</span><strong style={{ fontSize: 13 }}>{newOrderPreview.kind === "invoice" ? "XML da nota fiscal" : "Etiqueta de envio"}</strong><span style={{ marginLeft: 10, fontSize: 12, opacity: .8 }}>1&nbsp; / &nbsp;1</span><span style={{ marginLeft: 10, width: 1, height: 26, background: "rgba(255,255,255,.25)" }} />

...
type ProdutoFormProps = {
  ...
  defaultValues?: {
    ...
    commercialKitRules?: ProductCommercialKitRuleOption[];
  };
};
...
export function ProdutoForm({
  ...
  const [commercialKitRules, setCommercialKitRules] = useState<
    Array<ProductCommercialKitRuleOption & { key: string }>
  >(
    (defaultValues?.commercialKitRules ?? []).map((rule, index) => ({
      ...rule,
  ...
  return (
    <form action={formAction} className="flex flex-col flex-1 h-full w-full">
      ...
      {commercialKitRules.map((rule) => (
        <div key={rule.key}>
          <input type="hidden" name="commercialKitMatchText" value={rule.matchText} />
          <input
            type="hidden"
            name="commercialKitQuantity"
            value={String(rule.operationalQuantity)}
          />
        </div>
      ))}

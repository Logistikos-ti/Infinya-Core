...
export function InactivityWarningDialog({
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/65 px-4 backdrop-blur-sm">
      <div className="w-full max-w-[680px] rounded-[32px] border border-rose-200 bg-white p-6 shadow-2xl md:p-8">
        <div className="flex flex-col gap-5 md:grid md:grid-cols-[minmax(0,1fr)_220px] md:items-start md:gap-6">
          <div className="min-w-0">
            ...
            <h3 className="mt-3 text-[2rem] font-semibold leading-tight text-slate-950 md:text-[2.35rem]">
              {title}
            </h3>
            <p className="mt-3 text-base leading-7 text-slate-600 md:hidden">
              {mobileDescription ?? description}
            </p>
            <p className="mt-4 hidden text-lg leading-8 text-slate-600 md:block">{description}</p>
          </div>

          <div className="mx-auto w-full max-w-[240px] rounded-[28px] border border-rose-200 bg-rose-50 px-5 py-5 text-center md:mx-0 md:max-w-none md:px-6 md:py-6">
        ...
        <div className="mt-5 rounded-[24px] border border-amber-200 bg-amber-50 px-5 py-4 text-base leading-7 text-amber-900 md:mt-6 md:text-lg">

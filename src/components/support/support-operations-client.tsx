...
import { Check, ChevronDown, CircleHelp, Loader2, MessageCircle, Send, X } from "lucide-react";
...
export function SupportOperationsClient() {
  ...
  return (
    <div className="space-y-5">
      ...
      {selected ? <TicketDrawer selected={selected} comment={comment} statusOpen={statusOpen} sending={sending} onClose={() => { setSelected(null); setComment(""); setStatusOpen(false); }} onToggleStatus={() => setStatusOpen(!statusOpen)} onChangeStatus={changeStatus} onChangeComment={setComment} onSendComment={sendComment} /> : null}
    </div>
  );
}

function TicketDrawer({ selected, comment, statusOpen, sending, onClose, onToggleStatus, onChangeStatus, onChangeComment, onSendComment }: { selected: Ticket; comment: string; statusOpen: boolean; sending: boolean; onClose: () => void; onToggleStatus: () => void; onChangeStatus: (status: string) => void; onChangeComment: (value: string) => void; onSendComment: () => void }) {
  return <div className="fixed inset-0 z-50 flex justify-end"><style jsx>{`@keyframes supportOpsDrawerIn { from { transform: translateX(36px); opacity: 0; } to { transform: translateX(0); opacity: 1; } } @keyframes supportOpsFade { from { opacity: 0; } to { opacity: 1; } }`}</style><button type="button" aria-label="Fechar detalhes do chamado" onClick={onClose} className="absolute inset-0 bg-slate-950/55 backdrop-blur-[3px]" style={{ animation: "supportOpsFade .25s ease both" }} /><aside className="relative flex h-full w-[520px] max-w-[96vw] flex-col border-l border-slate-200 bg-white shadow-[-24px_0_60px_rgba(0,0,0,0.35)] dark:border-white/10 dark:bg-[#0c1526]" style={{ animation: "supportOpsDrawerIn .32s cubic-bezier(.3,1,.4,1) both" }}>
    ...
    <div className="border-t border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-[#0c1526]"><div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-2 focus-within:border-violet-400 dark:border-white/10 dark:bg-white/5"><input value={comment} onChange={(event) => onChangeComment(event.target.value)} onKeyDown={(event) => event.key === "Enter" && onSendComment()} placeholder="Escreva um comentário..." className="h-10 min-w-0 flex-1 bg-transparent px-2 text-sm outline-none" /><button type="button" onClick={onSendComment} disabled={!comment.trim() || sending} aria-label="Enviar comentário" className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-r from-blue-500 to-violet-500 text-white transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-40">{sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}</button></div></div>
  </aside></div>;
}

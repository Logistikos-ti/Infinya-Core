              ...
              {items.map((item, i) => {
                ...
                return (
                  <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", borderBottom: `1px solid ${t.border}`, background: rowBg, animation: itemFull ? "rowIn 0.3s ease" : "none" }}>
                    ...
                    {item.imageUrl ? (
                      <button type="button" onClick={() => setExpandedProductImage({ url: item.imageUrl!, name: item.name })} title="Ampliar foto do produto" style={{ width: 44, height: 44, flexShrink: 0, padding: 0, border: 0, borderRadius: 0, background: "transparent", overflow: "hidden", cursor: "zoom-in", transition: "transform .18s ease, box-shadow .18s ease" }} onMouseEnter={(event) => { event.currentTarget.style.transform = "translateY(-2px) scale(1.04)"; event.currentTarget.style.boxShadow = "0 8px 18px rgba(15,23,42,.18)"; }} onMouseLeave={(event) => { event.currentTarget.style.transform = ""; event.currentTarget.style.boxShadow = ""; }}>

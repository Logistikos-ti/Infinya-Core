...
import { ShippingCancellationScanPanel } from "@/components/shipping/shipping-cancellation-scan-panel";
...
export default async function ShippingCancellationDetailPage({
  params,
  searchParams,
  ...
  const { data: cancelamento } = await supabase
    ...
    .select(
      "id, status, motivo, aberto_em, requer_bipagem, pedido:pedidos_expedicao(id, codigo, numero_wms, cliente_nome, depositante:depositantes(nome))",
    )
    .eq("id", id)
    .maybeSingle();

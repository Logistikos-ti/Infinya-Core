    ...
    <div className="space-y-8 relative opacity-95">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="p-5 rounded-2xl bg-white/70 dark:bg-zinc-900/65 backdrop-blur-md shadow-sm border border-primary-500/40 dark:border-primary-500/30 border-l-4 border-l-primary-500 hover:border-primary-500 hover:shadow-lg hover:shadow-primary-500/20 transition-all group">
          ...
          <h3 className="text-3xl font-bold text-slate-800 dark:text-zinc-100 mt-2">
            {receivingStats[0].value}
          </h3>
          <p className="mt-2 flex items-center gap-2 text-xs text-slate-500">
            {receivingStats[0].help}
          </p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-white/70 dark:bg-zinc-900/65 backdrop-blur-md shadow-sm border border-emerald-500/40 dark:border-emerald-500/30 border-l-4 border-l-emerald-500 hover:border-emerald-500 hover:shadow-lg hover:shadow-emerald-500/20 transition-all group">
          ...
          <h3 className="text-3xl font-bold text-slate-800 dark:text-zinc-100 mt-2">
            {receivingStats[3].value}
          </h3>
          <p className="mt-2 flex items-center gap-2 text-xs text-slate-500">
            {receivingStats[3].help}
          </p>
        </div>
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 border-b border-slate-200 dark:border-zinc-800 pb-2">
        <div className="flex overflow-x-auto no-scrollbar gap-2">
          <Link
            href="/recebimento"
            className={`px-4 py-2 font-medium text-sm whitespace-nowrap rounded-lg ${!statusFilter ? "bg-primary-500/10 text-primary-600 dark:text-primary-400" : "text-slate-500 hover:bg-slate-100 dark:hover:bg-zinc-800"}`}
          >
            Todas as entradas
          </Link>
          <Link
            href="/recebimento?status=AGUARDANDO"
            className={`px-4 py-2 font-medium text-sm whitespace-nowrap rounded-lg ${statusFilter === "AGUARDANDO" ? "bg-primary-500/10 text-primary-600 dark:text-primary-400" : "text-slate-500 hover:bg-slate-100 dark:hover:bg-zinc-800"}`}
          >
            Aguardando doca
          </Link>
          <Link
            href="/recebimento?status=EM_RECEBIMENTO"
            className={`px-4 py-2 font-medium text-sm whitespace-nowrap rounded-lg ${statusFilter === "EM_RECEBIMENTO" ? "bg-primary-500/10 text-primary-600 dark:text-primary-400" : "text-slate-500 hover:bg-slate-100 dark:hover:bg-zinc-800"}`}
          >
            {"Em Confer\u00eancia"}
          </Link>
          <Link
            href="/recebimento?status=DIVERGENCIA"
            className={`px-4 py-2 font-medium text-sm whitespace-nowrap rounded-lg ${statusFilter === "DIVERGENCIA" ? "bg-primary-500/10 text-primary-600 dark:text-primary-400" : "text-slate-500 hover:bg-slate-100 dark:hover:bg-zinc-800"}`}
          >
            {"Com Diverg\u00eancia"}
          </Link>
        </div>
        ...
        <form className="flex items-center gap-2 flex-wrap">
          ...
          <button
            type="submit"
            className="p-2 bg-slate-900 dark:bg-zinc-800 text-white border border-slate-900 dark:border-zinc-700 rounded-lg hover:bg-slate-800 transition flex items-center justify-center"
          >
          ...
          {(statusFilter || depositanteFilter || dateFrom || dateTo) && (
            <Link
              href="/recebimento"
              className="text-xs text-slate-500 hover:text-slate-700 dark:hover:text-zinc-300 underline underline-offset-2"
            >
              Limpar
            </Link>
          )}
        ...
        <div className="p-5 border-b border-slate-200 dark:border-zinc-800/50 flex justify-between items-center">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Exibindo {visibleStart}-{visibleEnd} de {totalOrders}{" "}
              recebimento(s)
            </p>
          </div>
          <div className="flex items-center gap-3">
            <form className="flex items-center gap-2">
              ...
              <input
                type="hidden"
                name="depositante"
                value={effectiveDepositanteFilter}
              />
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-zinc-800/50 text-slate-700 dark:text-zinc-300">
              {paginatedOrders.map((order) => (
                <tr
                  key={order.id}
                  className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/20 transition group"
                >
                  <td className="px-6 py-4">
                    <Link
                      href={`/recebimento/${order.id}`}
                      className="font-bold text-slate-900 dark:text-white hover:text-primary-600 dark:hover:text-primary-400"
                    >
                      {order.code}
                    </Link>
                  </td>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
          ...
          <div className="mt-4 space-y-3">
            {operationalIssues.length ? (
              operationalIssues.map((issue) => (
                <div
                  key={issue.id}
                  className="rounded-xl bg-rose-500/10 border border-rose-500/20 p-4"
                >
                  ...
                  <p className="mt-1 text-sm font-medium text-rose-800 dark:text-rose-300">
                    {issue.action}
                  </p>
                </div>
              ))
        ...
        <div className="rounded-2xl bg-white/70 dark:bg-zinc-900/65 backdrop-blur-md shadow-sm border border-zinc-200/80 dark:border-zinc-800/80 p-6 hover:border-accent-500/30 transition-all">
          <h2 className="text-lg font-semibold text-slate-950 dark:text-white flex items-center gap-2">
            <ClipboardList className="w-5 h-5 text-accent-500" /> Tarefas de
            doca
          </h2>
          <div className="mt-4 space-y-3">
            {receivingTasks.map((task) => (
              <div
                key={task.id}
                className="rounded-xl border border-slate-200 dark:border-zinc-800 p-4 hover:border-slate-300 dark:hover:border-zinc-700 transition"
              >
...
function StatusBadge({ status }: { status: string }) {
  if (status === "AGUARDANDO") {
    return (
      <span className="px-2.5 py-1 rounded-md text-xs font-bold bg-primary-500/10 text-primary-600 dark:text-primary-400 border border-primary-500/20">
        Aguardando doca
      </span>
    );
  }
  ...
  if (status === "RECEBIDO" || status === "RECEBIDO_PARCIAL") {
    return (
      <span className="px-2.5 py-1 rounded-md text-xs font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
        {status === "RECEBIDO" ? "Finalizado" : "Rec. Parcial"}
      </span>
    );
  }
  if (status === "CANCELADO") {
    return (
      <span className="px-2.5 py-1 rounded-md text-xs font-bold bg-slate-500/10 text-slate-600 dark:text-slate-400 border border-slate-500/20">
        Cancelado
      </span>
    );
  }

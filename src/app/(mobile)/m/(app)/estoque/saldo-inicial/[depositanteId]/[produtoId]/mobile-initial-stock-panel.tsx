...
type Endereco = { id: string; codigo: string; area: string };
...
export function MobileInitialStockPanel({
  depositanteId,
  depositanteNome,
  produtoNome,
  produtoSku,
  ...
  function applyScan(rawValue: string) {
    ...
    if (scanPhase === "produto") {
      ...
      flash({ type: "ok", title: "Produto OK", code: produtoSku, sub: "Bipe agora o endereço de destino" });
      setScanPhase("endereco");
      return;
    }

    if (scanPhase === "endereco") {
      const match = enderecos.find((item) => normalizeScan(item.codigo) === normalized);
      if (!match) {
        flash({ type: "err", title: "Endereço não encontrado", code: rawValue, sub: "Este código não corresponde a nenhum endereço ativo." });
        return;
      }
      flash({ type: "ok", title: "Endereço OK", code: match.codigo, sub: "Informe a quantidade a lançar" });
      setEndereco(match);
      if (closeTimerRef.current) window.clearTimeout(closeTimerRef.current);
  ...
  async function handleConfirm() {
    if (isSaving || !endereco || count <= 0) return;
    if (exigeLote && !lote.trim()) {
      setError("Este produto exige lote.");
      return;
    }
    if (exigeValidade && !validadeEm) {
      setError("Este produto exige validade.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/estoque", {
        ...
        body: JSON.stringify({
          ...
          produtoCodigo: produtoSku,
          quantidade: count,
          lote: lote.trim() || undefined,
          validadeEm: validadeEm || undefined,
        }),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? "Não foi possível lançar o saldo inicial.");

      playFeedback("ok");
      setOverlay({ type: "ok", title: "Lançado!", code: `${count} un`, sub: `${produtoNome} em ${endereco.codigo}` });
      overlayTimerRef.current = window.setTimeout(() => {
        setOverlay(null);
        setConfirmed(true);
      }, FLASH_DURATION_MS);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao lançar o saldo inicial.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  const scanTitle = scanPhase === "produto" ? "Bipe o produto" : scanPhase === "endereco" ? "Bipe o endereço de destino" : "Informe a quantidade";
  ...
  return (
    <div className="relative flex flex-col" style={{ flex: 1, minHeight: 0 }}>
      ...
      <div className="flex shrink-0 items-center gap-3 px-[18px] pb-3 pt-[18px]">
        <MobileBackButton onClick={() => router.push(`/m/estoque/saldo-inicial/${depositanteId}`)} />
        ...
        {confirmed ? (
          <div className="mt-8 flex flex-1 flex-col items-center justify-center gap-4 text-center">
            ...
            <span className="max-w-[260px] text-[13.5px] leading-relaxed" style={{ color: mobileColors.muted }}>
              {count} un de {produtoNome} lançadas em {endereco?.codigo}.
            </span>
            <button
              type="button"
              onClick={() => router.push(`/m/estoque/saldo-inicial/${depositanteId}`)}
              className="mt-2 flex h-[52px] items-center justify-center gap-2 rounded-[15px] px-6 text-[14.5px] font-bold"
              style={{ border: `1px solid ${hexAlpha(mobileColors.cyan, 0.4)}`, color: mobileColors.cyanLight }}
            >
              ...
              >
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                  <span className="text-[11px] uppercase tracking-wide" style={{ color: mobileColors.muted }}>
                    {scanPhase === "produto" ? "Código de barras" : "Qualquer endereço ativo"}
                  </span>
                  <span className="truncate text-[22px] font-bold tracking-wide" style={{ color: mobileColors.text, ...headingFont }}>
                    {scanPhase === "produto" ? produtoBarcode || produtoSku : "—"}
                  </span>
                </div>
              </div>
            ) : (
              <div className="flex flex-col gap-3.5">
                ...
                >
                  <span className="text-center text-[12.5px] font-bold" style={{ color: mobileColors.muted }}>
                    Quantidade a lançar em {endereco?.codigo}
                  </span>
      ...
      {!confirmed && scanPhase === "quantidade" ? (
        ...
        >
          <button
            ...
            disabled={isSaving || editingCount || count <= 0}
          ...
          >
            {isSaving ? <MobileButtonSpinner /> : "Lançar estoque"}
          </button>
        </div>
      ) : null}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 3, minWidth: 0 }}>
              ...
              <span
                style={{
                  color: "#fff",
                  fontWeight: 800,
                  fontSize: scanPhase === "produto" ? 17 : 22,
                  lineHeight: 1.15,
                  display: "-webkit-box",
              ...
              >
                {scanPhase === "produto" ? produtoNome : "Qualquer endereço ativo"}
              </span>
              {scanPhase === "produto" ? (

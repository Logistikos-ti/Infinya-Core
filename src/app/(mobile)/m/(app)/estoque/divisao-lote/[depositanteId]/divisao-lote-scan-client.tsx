...
export function DivisaoLoteScanClient({
  depositanteId,
  depositanteNome,
}: {
  ...
  async function applyScan(rawValue: string) {
    ...
    try {
      const response = await fetch("/api/estoque/resolver-produto", {
        ...
        body: JSON.stringify({ depositanteId, barcode: code, mode: "divisao-lote" }),
      });
      ...
      if (matches.length === 1) {
        ...
        stopCamera(null);
        window.setTimeout(() => {
          router.push(`/m/estoque/divisao-lote/${depositanteId}/${matches[0].estoqueId}`);
        }, FLASH_DURATION_MS);
        return;
      }
  ...
  if (chooser) {
    return (
      <MobileListShell
        ...
        subtitle="Escolha o lote que será dividido"
        count={chooser.matches.length === 1 ? "1 lote" : `${chooser.matches.length} lotes`}
        onBack={() => router.push(`/m/estoque/divisao-lote/${depositanteId}`)}
        items={chooser.matches.map((match) => ({
          icon: "loc",
          iconColor: mobileColors.cyan,
          title: `${match.enderecoCodigo} • Lote ${match.lote ?? "sem lote"}`,
          tag: `${match.disponivel} un`,
          tagColor: mobileColors.cyan,
          sub: `Validade ${formatValidade(match.validadeEm)} • Sistema: ${match.quantidade} un`,
          onClick: () => router.push(`/m/estoque/divisao-lote/${depositanteId}/${match.estoqueId}`),
        }))}
      />
    );
  }

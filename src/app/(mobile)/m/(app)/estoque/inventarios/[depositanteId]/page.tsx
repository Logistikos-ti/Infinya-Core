...
type RelationName =
  | { nome?: string; sku?: string; codigo?: string; imagem_principal_url?: string | null }
  | Array<{ nome?: string; sku?: string; codigo?: string; imagem_principal_url?: string | null }>
  | null;

function extractField(value: RelationName, field: "nome" | "sku" | "codigo" | "imagem_principal_url") {
...
export default async function MobileStockInventarioProdutosPage({
  ...
  const { data: estoqueRows } = await adminSupabase
    .from("estoque")
    .select("id, quantidade, produto:produtos(nome, sku, imagem_principal_url), endereco:enderecos(codigo)")

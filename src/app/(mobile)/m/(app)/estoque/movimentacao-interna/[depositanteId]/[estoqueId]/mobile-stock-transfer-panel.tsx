...
export function MobileStockTransferPanel({
  ...
  function applyScan(rawValue: string) {
    ...
    if (scanPhase === "origem") {
      ...
      if (!expected || normalized !== expected) {
        flash({ type: "err", title: "Endereço incorreto", code: rawValue, sub: "Bipe o endereço de origem indicado na tela." });
        return;
      }
      flash({ type: "ok", title: "Origem OK", code: origemCodigo, sub: "Bipe agora o endereço de destino" });
      setScanPhase("destino");
      return;
    }

    if (scanPhase === "destino") {
      if (normalized === normalizeScan(origemCodigo)) {
        flash({ type: "err", title: "Mesmo endereço", code: rawValue, sub: "Escolha um destino diferente da origem." });
        return;
      }
      const match = destinos.find((item) => normalizeScan(item.codigo) === normalized);
      if (!match) {
        flash({ type: "err", title: "Endereço não encontrado", code: rawValue, sub: "Este código não corresponde a nenhum endereço ativo." });
        return;
      }
      flash({ type: "ok", title: "Destino OK", code: match.codigo, sub: "Informe a quantidade a mover" });
      setDestino(match);
      if (closeTimerRef.current) window.clearTimeout(closeTimerRef.current);
  ...
  function commitCountInput() {
    ...
    const clamped = Number.isFinite(parsed) ? Math.min(disponivel, Math.max(0, Math.round(parsed))) : 0;
    setCount(clamped);
    setEditingCount(false);
  }

  async function handleConfirm() {
    if (isSaving || !destino || count <= 0) return;
    setIsSaving(true);
    setError(null);
    ...
    try {
      const response = await fetch("/api/estoque/movimentacoes", {
        ...
        body: JSON.stringify({
          ...
          depositanteId,
          stockId: estoqueId,
          destinationAddressId: destino.id,
          quantity: count,
        }),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? "Não foi possível concluir a transferência.");

      playFeedback("ok");
      setOverlay({ type: "ok", title: "Transferido!", code: `${count} un`, sub: `${origemCodigo} → ${destino.codigo}` });
      overlayTimerRef.current = window.setTimeout(() => {
        setOverlay(null);
        setConfirmed(true);
      }, FLASH_DURATION_MS);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao registrar a transferência.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return (
    <div className="relative flex flex-col" style={{ flex: 1, minHeight: 0 }}>
      ...
      <div className="flex shrink-0 items-center gap-3 px-[18px] pb-3 pt-[18px]">
        <MobileBackButton onClick={() => router.push(`/m/estoque/movimentacao-interna/${depositanteId}`)} />
        ...
        {confirmed ? (
          <div className="mt-8 flex flex-1 flex-col items-center justify-center gap-4 text-center">
            ...
            <span className="max-w-[260px] text-[13.5px] leading-relaxed" style={{ color: mobileColors.muted }}>
              {count} un de {produtoNome} movidas de {origemCodigo} para {destino?.codigo}.
            </span>
          </div>
        ) : (
          <div
            ...
            {scanPhase !== "quantidade" ? (
              ...
              >
                ...
                <span className="text-center text-[11.5px]" style={{ color: mobileColors.dim }}>
                  Disponível para mover: {disponivel} un · destino {destino?.codigo} · toque no número para digitar
                </span>
              </div>
            )}
          </div>
      ...
      {!confirmed && scanPhase === "quantidade" ? (
        ...
        >
          ...
          >
            {isSaving ? <MobileButtonSpinner /> : "Confirmar transferência"}
          </button>
        </div>
      ) : null}
      ...
      {scannerOpen ? (
        <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "#000", display: "flex", flexDirection: "column" }}>
          ...
          >
            ...
            <button
              type="button"
              onClick={() => router.push(`/m/estoque/movimentacao-interna/${depositanteId}`)}

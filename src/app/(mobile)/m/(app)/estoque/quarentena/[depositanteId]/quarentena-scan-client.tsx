...
export function QuarentenaScanClient({
  depositanteId,
  depositanteNome,
}: {
  ...
  async function applyScan(rawValue: string) {
    ...
    try {
      ...
      if (matches.length === 1) {
        ...
        stopCamera(null);
        window.setTimeout(() => {
          router.push(`/m/estoque/quarentena/${depositanteId}/${matches[0].estoqueId}`);
        }, FLASH_DURATION_MS);
        return;
      }
  ...
  if (chooser) {
    return (
      <MobileListShell
        ...
        onBack={() => router.push(`/m/estoque/quarentena/${depositanteId}`)}
        items={chooser.matches.map((match) => ({
          ...
          onClick: () => router.push(`/m/estoque/quarentena/${depositanteId}/${match.estoqueId}`),
        }))}
      />
    );
  }

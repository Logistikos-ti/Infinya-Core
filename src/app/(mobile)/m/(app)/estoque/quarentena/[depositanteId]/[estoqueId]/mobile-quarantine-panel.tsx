...
export function MobileQuarantinePanel({
  depositanteId,
  depositanteNome,
  estoqueId,
  produtoNome,
  ...
  async function handleConfirm() {
    ...
    if (!cleanReason) {
      setError("Informe o motivo da quarentena.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error ?? "Não foi possível enviar para quarentena.");

      playFeedback("ok");
      setOverlay({
        type: "warn",
        title: "Quarentena criada!",
        code: `${count} un`,
        sub: `${produtoNome} — ${enderecoCodigo}`,
      });
      ...
      playFeedback("err");
      setError(err instanceof Error ? err.message : "Falha ao enviar para quarentena.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return (
    <div className="relative flex flex-col" style={{ flex: 1, minHeight: 0 }}>
      ...
      <div className="flex shrink-0 items-center gap-3 px-[18px] pb-3 pt-[18px]">
        <MobileBackButton onClick={() => router.push(`/m/estoque/quarentena/${depositanteId}`)} />
        ...
        >
          {confirmed ? "Retido" : enderecoCodigo}
        </span>
      </div>
          ...
          <div className="mt-8 flex flex-1 flex-col items-center justify-center gap-4 text-center">
            <div className="relative flex h-24 w-24 items-center justify-center">
              <span
                ...
                style={{ border: `2px solid ${mobileColors.amber}`, animation: "mobileRingPulse 1.6s ease-out infinite" }}
              />
              <span
                className="flex h-20 w-20 items-center justify-center rounded-full"
                style={{ background: hexAlpha(mobileColors.amber, 0.16), color: mobileColors.amber }}
              >
                <MobileIcon name="shield" size={40} strokeWidth={2.6} />
              </span>
            </div>
            ...
            <span className="max-w-[270px] text-[13.5px] leading-relaxed" style={{ color: mobileColors.muted }}>
              {count} un de {produtoNome} foram retiradas do saldo disponível para análise.
            </span>
            <div className="mt-2 flex w-full flex-col gap-2.5">
              <button
                type="button"
                onClick={() => router.push(`/m/estoque/quarentena/${depositanteId}`)}
          ...
          >
            ...
            <label className="flex flex-col gap-2">
              ...
              <textarea
                ...
                placeholder="Ex.: avaria, conferência fiscal, item suspeito..."
                className="min-h-[112px] resize-none rounded-[15px] px-4 py-3 text-[15px] outline-none"
                style={{
                  background: "rgba(5,7,13,0.5)",
                  border: `1px solid ${hexAlpha(reason.trim() ? mobileColors.green : mobileColors.amber, 0.32)}`,
      ...
      {!confirmed ? (
        ...
        >
          <button
            ...
            disabled={isSaving || editingCount || count <= 0 || !reason.trim()}
          ...
          >
            {isSaving ? <MobileButtonSpinner /> : "Enviar para quarentena"}
          </button>
        </div>
      ) : null}
    </div>

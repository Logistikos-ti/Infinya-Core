 ...
 * A product stored in a single location goes straight into the exit
 ...
 */
export function SaidaManualScanClient({
  depositanteId,
  depositanteNome,
}: {
  ...
  async function applyScan(rawValue: string) {
    ...
    try {
      const response = await fetch("/api/estoque/resolver-produto", {
        ...
        body: JSON.stringify({ depositanteId, barcode: code, mode: "saida" }),
      });
      ...
      if (matches.length === 1) {
        ...
        stopCamera(null);
        window.setTimeout(() => {
          router.push(`/m/estoque/saida-manual/${depositanteId}/${matches[0].estoqueId}`);
        }, FLASH_DURATION_MS);
        return;
      }
  ...
  if (chooser) {
    return (
      <MobileListShell
        ...
        onBack={() => router.push(`/m/estoque/saida-manual/${depositanteId}`)}
        items={chooser.matches.map((match) => ({
          ...
          onClick: () => router.push(`/m/estoque/saida-manual/${depositanteId}/${match.estoqueId}`),
        }))}
      />
    );
  }

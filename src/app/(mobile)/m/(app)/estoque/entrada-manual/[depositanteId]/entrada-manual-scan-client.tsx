 ...
 * A product stored in a single location goes straight into the entry
 * panel; a product split across more than one location shows a short
 * chooser -- just that product's own addresses, not the full catalog.
 */
export function EntradaManualScanClient({
  depositanteId,
  depositanteNome,
}: {
  ...
  async function applyScan(rawValue: string) {
    ...
    try {
      const response = await fetch("/api/estoque/resolver-produto", {
        ...
        body: JSON.stringify({ depositanteId, barcode: code, mode: "entrada" }),
      });
      ...
      if (matches.length === 1) {
        ...
        stopCamera(null);
        window.setTimeout(() => {
          router.push(`/m/estoque/entrada-manual/${depositanteId}/${matches[0].estoqueId}`);
        }, FLASH_DURATION_MS);
        return;
      }
  ...
  if (chooser) {
    return (
      <MobileListShell
        ...
        onBack={() => router.push(`/m/estoque/entrada-manual/${depositanteId}`)}
        items={chooser.matches.map((match) => ({
          icon: "loc",
          iconColor: mobileColors.green,
          title: match.enderecoCodigo,
          tag: `${match.quantidade} un`,
          tagColor: mobileColors.green,
          sub: `Saldo atual`,
          onClick: () => router.push(`/m/estoque/entrada-manual/${depositanteId}/${match.estoqueId}`),
        }))}
      />
    );
  }

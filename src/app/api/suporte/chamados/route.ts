...
export async function GET() {
  ...
  if (ticketError || commentError || depositanteError) {
    return NextResponse.json({ error: ticketError?.message ?? commentError?.message ?? depositanteError?.message ?? "Não foi possível carregar os chamados." }, { status: 500 });
  }

  const commentsByTicket = new Map<string, Array<{ id: string; chamado_id: string; texto: string; created_at: string; autor_id: string }>>();
  ...
  return NextResponse.json({ tickets: (tickets ?? []).map((ticket) => serializeTicket({ ...ticket, depositante: { nome: depositanteNames.get(ticket.depositante_id) ?? null }, comentarios: commentsByTicket.get(ticket.id) ?? [] })) });
}

export async function POST(request: NextRequest) {
  const auth = await requireApiRoleAccess(SUPPORT_ROLES);
  if (auth.response) return auth.response;
  const body = (await request.json().catch(() => null)) as { subject?: unknown; message?: unknown; category?: unknown } | null;
  ...
  const { error: commentError } = await supabase.from("suporte_comentarios").insert({ chamado_id: ticket.id, autor_id: auth.user.id, texto: message });
  if (commentError) return NextResponse.json({ error: commentError.message }, { status: 500 });

  return NextResponse.json({ ticket: serializeTicket({ ...ticket, depositante: { nome: auth.user.depositanteNome }, comentarios: [{ id: crypto.randomUUID(), texto: message, created_at: new Date().toISOString(), autor: { nome: auth.user.nome, papel: auth.user.papel } }] }) }, { status: 201 });
}
...
function serializeTicket(ticket: any) {
  ...
  return {
    ...
    tone,
    comments: comments.map((comment: any) => ({ id: comment.id, text: comment.texto, author: comment.autor?.nome ?? "Usuário", role: comment.autor?.papel ?? null, createdAt: comment.created_at })),

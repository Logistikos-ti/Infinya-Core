...
export async function PATCH(request: Request, context: Context) {
  ...
  const payload = (await request.json().catch(() => null)) as { quantidade?: number | string } | null;
  ...
  try {
    ...
    const result = await recordGeneralInventoryItem({ inventoryId: id, itemId, userId: auth.user.id, quantidade });

...
async function listProds() {
  const { data: prods } = await supabase.from('produtos').select('id, sku, nome, depositante_id, codigo_externo').limit(10);
  console.log("Produtos:", prods);

  const { data: deps } = await supabase.from('depositantes').select('id, nome').limit(10);
  console.log("Depositantes:", deps);
}

listProds().catch(console.error);
